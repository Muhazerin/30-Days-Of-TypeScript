...
let numBtnZeroAndOneHundred: number = Math.floor(Math.random() * 101)
...
let numBtnZeroAndFifty: number = Math.floor(Math.random() * 51);
let numBtnFiftyAndOneHundred: number = numBtnZeroAndFifty + 50;
...
let numBtnZeroAnd255: number = Math.floor(Math.random() * 256);
...
let index: number = Math.floor(Math.random() * javascript.length);
...
let phrase: string = sentence.substring(31, 54);
console.log(`\n12. Phrase: ${phrase}`)
